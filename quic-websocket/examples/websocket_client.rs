...
#[derive(Parser, Debug)]
#[command(author, version, about = "Enhanced QUIC WebSocket client with subscription support")]
struct Args {
    ...
    /// Client name
    #[arg(short, long, default_value = "WebSocketUser")]
    name: String,
...
async fn main() -> Result<()> {
    ...
    println!("🚀 Enhanced QUIC WebSocket Client");
    ...
    println!();
    println!("💬 Enhanced WebSocket Commands:");
    println!("  /subscribe <topics>   - Subscribe to topics (comma-separated)");
    println!("  /unsubscribe <topics> - Unsubscribe from topics");
    ...
    println!("  • system      - System notifications");
    println!("  • sensors     - Sensor data (temperature, humidity, pressure)");
    println!("  • monitoring  - System monitoring (CPU, memory, disk)");
    println!("  • stocks      - Stock price updates");
    println!("  • news        - News and announcements");
    println!("  • tips        - Usage tips");
    println!("  • tech        - Technical information");
    println!("  • security    - Security updates");
    println!("  • performance - Performance insights");
    println!();
    println!("💡 Example: /subscribe sensors,stocks,system");
...
async fn handle_user_input(connection: &quinn::Connection, input: &str) -> Result<()> {
    if input.starts_with('/') {
        ...
        match command {
            "/subscribe" => {
                if args.is_empty() {
                    println!("Usage: /subscribe <topics> (comma-separated)");
                    println!("Example: /subscribe sensors,stocks,system");
                    return Ok(());
                }
                let topics: Vec<String> = args.split(',')
                    .map(|s| s.trim().to_string())
                    .filter(|s| !s.is_empty())
                    .collect();
                
                let subscribe = MessageFrame::new(MessageType::Subscribe { topics });
                send_message(connection, &subscribe).await?;
            }
            "/unsubscribe" => {
                if args.is_empty() {
                    println!("Usage: /unsubscribe <topics> (comma-separated)");
                    return Ok(());
                }
                let topics: Vec<String> = args.split(',')
                    .map(|s| s.trim().to_string())
                ...
                let unsubscribe = MessageFrame::new(MessageType::Unsubscribe { topics });
                send_message(connection, &unsubscribe).await?;
            }
            "/broadcast" => {
                if args.is_empty() {
            ...
            _ => {
                ...
                println!("Available commands: /subscribe, /unsubscribe, /broadcast, /list, /ping, /quit");
            }
        }
    } else {
        // Regular message
...
async fn handle_incoming_message(recv_stream: &mut quinn::RecvStream) -> Result<()> {
    ...
    match &frame.message_type {
        ...
        MessageType::ServerPush { topic, content, .. } => {
            println!("📡 [{}] {}", topic.to_uppercase(), content);
        }
